...
Before writing a carrier, Package requires shared Tooling preview/output to match the exact carrier dimension and Workspace selection. Pointerless Workspace carriers pass the visible host-projected ZIP filename into shared Tooling and verify that exact filename on preview and manufacture. Routed Handoff carrier filenames are owned by shared Core: VS Code first previews Core's exact basename, allocates any host-local collision suffix against that basename, re-previews when a suffix is needed, and requires manufacture to return the exact same Core filename. VS Code never renames or post-edits qualified carrier bytes. An existing destination ZIP is never silently replaced: byte-identical output may be reused, while a different payload at the same canonical filename fails closed and must be resolved explicitly.
...
8. `tiinex.git.postStagePolicy` is the single post-stage policy: **Do Nothing**, **Ask**, **Commit**, or **Commit + Push**. It is independent of `tiinex.landing.stage` and is driven by the stable staged state itself, so manually staged Tiinex Markdown material participates even when Receive did not stage it. Automatic evaluation never stages extra material: it requires a conflict-free exact staged fingerprint, no relevant unstaged remainder, and at least one staged Tiinex Markdown artifact under `.topics/`; it does **not** run Tiinex qualification as a commit gate. **Ask** prompts once for an unchanged staged Tiinex state with exactly **Commit**, **Commit + Push**, and **Cancel**. Cancel leaves staging unchanged; Commit creates only the derived local commit; Commit + Push requires the existing aligned-upstream safety and pushes only the exact commit created by that prompt. **Commit + Push** additionally requires an already aligned upstream before the commit; push is then allowed only for the exact commit created by that same Tiinex operation. Source-only staging and manual/unrelated commits are never auto-pushed.
...
`Tiinex: New Artifact` is the primary authoring entrypoint and is visible directly on the **Discovery** tree title in the Tiinex Activity surface. **Tiinex: New Feedback** and **Tiinex: New Handoff** sit beside it as shortcuts that preselect `tiinex.feedback.v1` and `tiinex.handoff.v1` through the same generic flow; they do not introduce private host forms or path rules. The selected local Workspace is projected through installed Core, and the schema picker is populated only from the currently create-capable schemas returned by Core. A shortcut therefore fails closed if its exact schema is not create-capable in the installed Core instead of silently substituting host behavior. VS Code keeps no private schema allow-list.
...
The **New Feedback** and **New Handoff** actions are schema-preselection shortcuts through that same generic schema/model/Parent/path flow. Handoff-specific host behavior is intentionally limited to the optional **Attach to Outgoing** transport action, which is offered only when the selected local Workspace is already part of an open Outgoing context and runs only after the exact reviewed Handoff bytes have been written and re-qualified. Attaching does not alter artifact semantics or bytes. Existing qualified Handoffs can likewise be attached intentionally; **Copy Handoff Transport Text** copies forwarding text and does not imply recipient acceptance, Role authority or a new Handoff identity.
...
- `tiinex.git.postStagePolicy` is the one automatic post-stage setting: **Do Nothing**, **Ask** (default), **Commit**, or **Commit + Push**. `tiinex.landing.stage` remains independent and controls only whether successful Receive landing stages safe changes; manual Source Control staging is evaluated by the same watcher even when landing staging is disabled. A debounced Git repository-state watcher evaluates stable staged-state events and never performs an implicit Stage All. **Ask** prompts once per unchanged staged Tiinex fingerprint with **Commit**, **Commit + Push**, and **Cancel**; Cancel/dismissal leaves staging intact, Commit creates only the derived local commit, and Commit + Push uses the same exact-commit transport safety as the automatic push policy. For one explicit multi-repository Incoming Merge/Replace, that Ask decision is operation-scoped: one prompt covers all safely staged repositories from that operation, while `commit` and `commit-push` remain non-interactive.
- Automatic post-stage commit is a Git-history policy, not a qualification boundary. It requires no unresolved conflicts, no unstaged remainder, an unchanged branch/HEAD/index/working-state fingerprint across message derivation, and at least one staged Tiinex Markdown artifact under `.topics/`. Validator findings, an unstaged Parent, or other qualification state do not block ordinary commit; source-only staging remains outside automatic Tiinex policy.
...
`Ctrl+Shift+B` runs **Tiinex: Build linked extension** with the explicit sequence **npm install → in-place TypeScript build**. It deliberately does **not** rerun the link step: rewriting VS Code's extension registry immediately before a clean rebuild can make the host observe the extension while its compiled entrypoint is temporarily absent. The one-time **Link this checkout** task owns registry repair, is idempotent when the exact linked entry is already present, and should be rerun only when that registration itself needs repair. The development build also avoids deleting `dist/` before compilation, so the currently loaded entrypoint remains present until the new output is emitted. There is no per-edit VSIX manufacture and no Tiinex-owned reload marker protocol; VS Code observes the registered junction-backed extension and owns the restart-required UX.
