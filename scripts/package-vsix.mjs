...
for (const relative of await walk(path.join(ROOT, 'shared-core'))) {
  files.push([`extension/shared-core/${relative}`, await readFile(path.join(ROOT, 'shared-core', relative))]);
}
for (const relative of await walk(path.join(ROOT, 'media'))) {
  files.push([`extension/media/${relative}`, await readFile(path.join(ROOT, 'media', relative))]);
}
const contentTypes = `<?xml version="1.0" encoding="utf-8"?><Types xmlns="http://schemas.openxmlformats.org/package/2006/content-types"><Default Extension="json" ContentType="application/json"/><Default Extension="js" ContentType="application/javascript"/><Default Extension="map" ContentType="application/json"/><Default Extension="md" ContentType="text/markdown"/><Default Extension="txt" ContentType="text/plain"/><Default Extension="" ContentType="application/octet-stream"/><Override PartName="/extension.vsixmanifest" ContentType="text/xml"/></Types>`;
...
console.log(JSON.stringify({ status: 'ready', output: OUT, bytes: bytes.length, sha256: createHash('sha256').update(bytes).digest('hex'), entries: files.length }));
