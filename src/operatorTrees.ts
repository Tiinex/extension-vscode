...
function progressHeartbeat(progress: vscode.Progress<{ message?: string; increment?: number }>): { report(message: string): void; dispose(): void } {
  ...
  const render = () => {
    ...
    progress.report({ message: `${stage} · ${elapsed}s` });
  };
  const timer = setInterval(render, 5000);
  return {
    report(message: string) { stage = String(message || '').trim(); render(); },
...
interface CarrierEndpointRoleBinding {
  party: 'from' | 'to';
  label: string;
  reference: string;
  workspaceId: string;
...
interface OutgoingDraft {
  ...
  participants: ReadonlyArray<CarrierRoleParticipant>;
  participantSelections: PackageParticipantRole[];
  endpointRoles: CarrierEndpointRoleBinding[];
  participantProjectionState: PackageParticipantProjection['state'];
  participantProjectionDetail: string;
  from: string;
  to: string;
...
export class TiinexOperatorTrees implements vscode.Disposable {
  ...
  private localWorkspaceChoicesCache: { at: number; value: Promise<PackageWorkspaceChoice[]> } | null = null;
  ...
  private async parentArtifactForResource(
    ...
    if (!resource || resource.scheme !== 'file') return this.pickArtifactParent(choice.workspaceId, catalog);
    const info = await stat(resource.fsPath);
    if (info.isDirectory()) return this.pickArtifactParent(choice.workspaceId, catalog);
  ...
  async beginArtifactAuthoring(resource?: vscode.Uri, preselectedSchemaId = ''): Promise<void> {
    try {
      ...
      await vscode.window.showErrorMessage(`Tiinex artifact authoring blocked: ${detail}`);
    }
  }
  ...
  private registerCommands(): void {
    ...
    register('tiinex.outgoing.newHandoffFromArtifact', (node?: OperatorNode) => this.newOutgoingHandoffFromArtifact(node));
    ...
    return {
      ...
      outgoing: this.outgoing ? {
        ...
        drafts: this.outgoing.drafts.map((item) => ({
          ...
          participantSelections: item.participantSelections.map((role) => ({ label: role.label, reference: role.reference, workspaceId: role.workspaceId, path: role.path })),
  ...
  private async extensionHostAcceptanceAuthorHandoff(value: any = {}): Promise<any> {
    ...
    const submission: ArtifactAuthoringSubmission = {
      ...
      values: {
        ...
        From: fromEndpoint.label,
        'From Kind': fromEndpoint.kind,
        To: toEndpoint.label,
        'To Kind': toEndpoint.kind,
        Transfers: [{ name: 'host-authored-work', fields: { 'Transfer Kind': 'work-and-responsibility', Description: 'Exercise the bounded real-host authoring path with exact Core-projected endpoint reference authority.' } }],
        'Required Context': 'none',
        'Reference Context': 'none',
        'Retained Responsibilities': 'none',
        'Exclusions And Dependencies': 'none',
  ...
  private corruptExtensionHostAcceptanceParticipantSnapshot(): any {
    if (!extensionHostAcceptanceEnabled()) throw new Error('tiinex.extension-host.acceptance-disabled');
    if (!this.outgoing) throw new Error('tiinex.extension-host.outgoing-required');
    for (const item of this.outgoing.drafts) item.participants = [];
    recordExtensionHostAcceptanceEvent('participant-snapshot-corrupted', { routeCount: this.outgoing.drafts.length });
    this.outgoingProvider.refresh();
    return this.extensionHostAcceptanceSnapshot();
  }
  ...
  private async restoreTransportQueue(): Promise<void> {
    ...
    for (const record of stored) {
      ...
      try {
        restored.push(await this.qualifyTransportPackage(packagePath, Array.isArray(record.routeIds) ? record.routeIds : null, Number(record.addedAt || 0) || Date.now(), 'restore'));
      } catch {
  ...
  private async refreshTransportQueue(interactive = false): Promise<void> {
    ...
    for (const record of records) {
      try { refreshed.push(await this.qualifyTransportPackage(record.packagePath, record.routeIds, record.addedAt, 'refresh')); }
  ...
  private async queueBuiltTransportPackage(packagePath: string, focus = false): Promise<string> {
    try {
      // A freshly manufactured package is still re-opened through Core before
      // Transport accepts it. Persisted host receipts are never semantic authority.
  ...
  private async queueTransportPackage(packagePath: string, routeSelector = '', focus = false): Promise<void> {
    ...
    try {
      ...
      const qualified = await this.qualifyTransportPackage(resolved, existing?.routeIds ?? null, existing?.addedAt || Date.now(), 'queue');
      if (routeSelector) {
  ...
  private async qualifyTransportPackage(packagePath: string, routeIds: string[] | null, addedAt: number, source: 'queue' | 'refresh' | 'restore' = 'queue'): Promise<TransportPackageState> {
    ...
    try {
      ...
      recordExtensionHostAcceptanceEvent('transport-qualified', { source, packagePath: resolved, sha256, routeCount: routes.length });
      return {
  ...
  private async selectOutgoingFolder(defaultFolder?: string, persist = true): Promise<string> {
    const acceptanceFolder = extensionHostAcceptanceOutgoingFolder();
  ...
  private async resolveIncomingTargetChoices(state: IncomingState, workspaceIds: string[], forcedStrategy?: IncomingApplyStrategy): Promise<PackageWorkspaceChoice[] | null> {
...
does not qualify as the selected Tiinex Workspace ${workspaceId}. No source was changed.`,
  ...
  private async prepareIncomingRepositorySession(state: IncomingState, workspaceIds: string[], forcedStrategy?: IncomingApplyStrategy): Promise<'ready' | 'deferred' | 'cancelled'> {
    const targets = await this.resolveIncomingTargetChoices(state, workspaceIds, forcedStrategy);
...
Tiinex will open a dedicated temporary multi-root workspace in a new VS Code window containing only these repositories. The current window/workspace is not modified, and no repository source changes occur before the resumed Review Plan reaches its final Execute step.`,
  ...
  private async pickOutgoingParentTopology(packagePath: string): Promise<{ routePointer: string; routeId: string; consolidation: boolean } | null> {
    ...
    const selected = await vscode.window.showQuickPick([
      {
        label: `$(combine) Consolidate all ${routes.length} qualified parent routes`,
      ...
      ...routes.map((route, index) => ({
        label: `$(git-branch) Continue parent route ${index + 1} · ${route.from} → ${route.to}`,
      ...
      title: 'New Outgoing · Parent route topology',
      placeHolder: 'Consolidate the qualified parent batch or continue one exact parent route.',
      canPickMany: false,
      ignoreFocusOut: true
    });
  ...
  private async outgoingChildren(node?: OperatorNode): Promise<OperatorNode[]> {
    ...
    if (node.data.kind === 'workspaceArchive') {
      ...
      if (!workspace) return [];
      const [artifacts, rawArtifacts] = await Promise.all([
        this.outgoingWorkspaceArtifacts(workspace),
        this.rawWorkspaceArtifacts('outgoing', workspace.workspaceId, workspace.packagePath || '')
      ]);
      return this.decorateOutgoingNodes(this.fileArtifactRoots('outgoing', workspace.workspaceId, artifacts, workspace.packagePath || ''), workspace, rawArtifacts);
    }
    if (node.data.kind === 'workspace') {
      ...
      if (logical) return this.decorateOutgoingNodes(logical, workspace, rawArtifacts);
      const artifacts = await this.outgoingWorkspaceArtifacts(workspace);
      return this.decorateOutgoingNodes(this.artifactProjectionChildren('outgoing', node, artifacts), workspace, rawArtifacts);
    }
    const workspace = this.outgoing.workspaces.find((item) => item.workspaceId === node.data.workspaceId);
    const logical = await this.logicalWorkspaceProjectionChildren('outgoing', node);
    const rawArtifacts = workspace ? await this.rawWorkspaceArtifacts('outgoing', workspace.workspaceId, workspace.packagePath || '') : [];
    if (logical) return workspace ? this.decorateOutgoingNodes(logical, workspace, rawArtifacts) : logical;
    const projected = this.artifactProjectionChildren('outgoing', node, await this.artifactsForNode('outgoing', node));
    return workspace ? this.decorateOutgoingNodes(projected, workspace, rawArtifacts) : projected;
  }

  private decorateOutgoingNodes(nodes: OperatorNode[], workspace: OutgoingWorkspace, rawArtifacts: IndexedArtifact[]): OperatorNode[] {
    const leaves = leafArtifactPathSet(rawArtifacts);
    for (const node of nodes) {
      const artifact = node.data.artifact;
      if (!artifact || !node.data.workspaceId || !leaves.has(normalizePath(artifact.path))) continue;
      const isHandoff = artifact.schemaId === 'tiinex.handoff.v1';
      const tracked = isHandoff ? this.outgoingDraftForPath(node.data.workspaceId, artifact.path) : undefined;
      let contextValue = node.data.contextValue || '';
      if (workspace.source === 'local') {
        contextValue = isHandoff
          ? (tracked?.routeIncluded ? 'tiinex.outgoingLocalHandoffAttached' : 'tiinex.outgoingLocalHandoffUnattached')
          : 'tiinex.outgoingLocalArtifactLeaf';
      } else if (isHandoff) {
        contextValue = tracked?.routeIncluded ? 'tiinex.outgoingIncomingHandoffAttached' : 'tiinex.outgoingIncomingHandoffUnattached';
      }
      if (!contextValue) continue;
      if (tracked) {
        const participantDescription = participantPresentation({
          state: tracked.participantProjectionState,
          roles: [...tracked.participants],
          detail: tracked.participantProjectionDetail,
          findings: []
        }).description;
        const baseDescription = String(node.description || node.data.description || '').trim();
        const baseTooltip = String(node.tooltip || node.data.tooltip || artifact.path).trim();
        node.data.description = [baseDescription, participantDescription].filter(Boolean).join(' · ');
        node.data.tooltip = `${baseTooltip}\n${tracked.participantProjectionDetail}\n${ACTIVE_SPEAKER_NON_AUTHORITY}`;
        node.description = node.data.description;
        node.tooltip = node.data.tooltip;
      }
  ...
  private async localLeafContext(workspaceId: string): Promise<{ choice: PackageWorkspaceChoice; byPath: Map<string, IndexedArtifact>; leaves: Set<string> } | null> {
    ...
    const value = (async () => {
      try {
        const matches = (await this.localWorkspaceChoices()).filter((item) => item.workspaceId === key && item.root);
  ...
  private async decorateOutgoingWorkspaceFileNodes(nodes: OperatorNode[], workspaceId: string): Promise<OperatorNode[]> {
    ...
    const artifacts = await this.rawWorkspaceArtifacts('outgoing', workspace.workspaceId, workspace.packagePath || '');
    const byPath = new Map(artifacts.map((artifact) => [normalizePath(artifact.path), artifact]));
    for (const node of nodes) {
      if (node.data.kind !== 'file' || !node.data.filePath) continue;
      const artifact = byPath.get(normalizePath(node.data.filePath));
  ...
  private endpointRoleBindingsFromSubmission(submission: ArtifactAuthoringSubmission): CarrierEndpointRoleBinding[] {
    const out: CarrierEndpointRoleBinding[] = [];
    const selections = submission.endpointSelections || {};
  ...
  private trackOutgoingHandoff(workspace: OutgoingWorkspace, draft: PreparedArtifactDraft, writtenPath: string, from: string, to: string, projection: PackageParticipantProjection, origin: 'created' | 'existing', routeIncluded = true, endpointRoles: CarrierEndpointRoleBinding[] = [], participantSelections: PackageParticipantRole[] = []): OutgoingDraft {
    ...
    if (existing) {
      ...
      existing.participants = projection.roles;
      existing.participantSelections = participantSelections;
      existing.endpointRoles = endpointRoles;
      existing.participantProjectionState = projection.state;
    ...
    const tracked: OutgoingDraft = {
      ...
      writtenPath,
      routeIncluded,
      participants: projection.roles,
      participantSelections,
      endpointRoles,
      participantProjectionState: projection.state,
  ...
  private async projectOutgoingParticipants(workspace: OutgoingWorkspace, handoffPath: string, participantSelections: PackageParticipantRole[] = []): Promise<PackageParticipantProjection> {
    ...
    return vscode.window.withProgress(
      { location: vscode.ProgressLocation.Notification, title: 'Tiinex qualifying participant authority', cancellable: false },
      async (progress) => {
        progress.report({ message: 'Qualifying selected Workspace sources…' });
        ...
        return projectHandoffPackageParticipants(this.extensionPath, {
          routeId: routeChoiceKeyForHandoff(workspace.workspaceId, handoffPath),
          routeInputs: [{ routeId: routeChoiceKeyForHandoff(workspace.workspaceId, handoffPath), participantRoles: participantSelections }],
          workspaceIds: this.outgoing!.workspaces.map((item) => item.workspaceId),
          packageParentPath: this.outgoing!.packageParentPath || '',
  ...
  private async pickArtifactParent(targetWorkspaceId: string, targetCatalog?: ArtifactAuthoringCatalog): Promise<ArtifactDraftParent | null | undefined> {
    const choices = await this.localWorkspaceChoices();
    const workspaceItems = choices.map((choice) => ({
      label: choice.workspaceId,
      description: choice.repository || choice.ref || '',
      detail: choice.root,
      choice
    ...
    const selectedWorkspace = await vscode.window.showQuickPick([
      { label: 'New lineage root', description: 'No Parent', detail: 'Core allocates a new root in the selected target directory.', choice: null as PackageWorkspaceChoice | null },
      ...workspaceItems
    ], {
      title: 'New Tiinex Artifact · Parent Workspace',
      placeHolder: 'Choose no Parent or one qualified Local Workspace',
      canPickMany: false,
      ignoreFocusOut: true
    });
    if (!selectedWorkspace) return undefined;
    if (!selectedWorkspace.choice) return null;

    const choice = selectedWorkspace.choice;
    const mode = await vscode.window.showQuickPick([
      { label: 'Leaves', description: 'Recommended', detail: 'Only current lineage leaves in this Workspace.', value: 'leaves' as const },
      { label: 'Full lineage', description: 'All Parent candidates', detail: 'Browse the complete indexed lineage candidate set for this Workspace.', value: 'full' as const }
    ], {
      title: `New Tiinex Artifact · ${choice.workspaceId} Parent Scope`,
      placeHolder: 'Choose the amount of lineage to browse',
      canPickMany: false,
      ignoreFocusOut: true
    });
    if (!mode) return undefined;

    const catalog = choice.workspaceId === targetWorkspaceId && targetCatalog
      ? targetCatalog
      : await loadArtifactAuthoringCatalog(this.extensionPath, choice.root);
    const scopedParents = catalog.parents.filter((parent) => mode.value === 'full' || parent.role === 'loaded-leaf-candidate');
    const candidates = (await Promise.all(scopedParents.map(async (parent) => {
      let mtimeMs = 0;
      try { mtimeMs = (await stat(safeTarget(choice.root, normalizePath(parent.path)))).mtimeMs; } catch { /* keep catalog candidate searchable */ }
      return { parent, mtimeMs };
    })))
      .sort((a, b) => b.mtimeMs - a.mtimeMs || String(a.parent.title || a.parent.path).localeCompare(String(b.parent.title || b.parent.path), undefined, { sensitivity: 'base' }));

    if (!candidates.length) {
      await vscode.window.showInformationMessage(`No ${mode.value === 'leaves' ? 'leaf ' : ''}Parent candidates are available in ${choice.workspaceId}.`);
      return undefined;
    }

    // Catalog membership is discovery only. Native authoring requires the exact
    // selected Parent to pass the shared Core parent audit. Qualify it before the
    // authoring panel opens so a historical/degraded leaf cannot send the operator
    // through a full form only to fail at Create/Preview. If one candidate is
    // blocked, keep the operator in the Parent picker instead of terminating the
    // whole authoring flow.
    for (;;) {
      const selected = await vscode.window.showQuickPick(candidates.map(({ parent, mtimeMs }) => ({
        ...
        placeHolder: `${mode.label}; newest modified first. Type to search this Workspace only.`,
        canPickMany: false,
        ignoreFocusOut: true
      });
      if (!selected) return undefined;
      const parentPath = normalizePath(selected.parent.path);
      const reference = choice.workspaceId === targetWorkspaceId ? parentPath : `${choice.workspaceId}::${parentPath}`;
      const parentArtifact: ArtifactDraftParent = {
        path: parentPath,
      ...
      try {
        ...
        const detail = shortMessage(error);
        const action = await vscode.window.showWarningMessage(
          `Selected Parent is not qualified for native continuation. Choose another Parent.`,
          'Choose Another Parent',
          'Show Details',
          'Cancel'
        );
        if (action === 'Show Details') {
          await vscode.window.showErrorMessage(detail, { modal: true });
          continue;
        }
        if (action === 'Choose Another Parent') continue;
        return undefined;
      }
    }
  }
  ...
  private async attachOutgoingHandoffNode(node?: OperatorNode): Promise<void> {
    ...
    try {
      ...
      recordExtensionHostAcceptanceEvent('attach-rejected', { message: shortMessage(error), workspaceId: String(node?.data.workspaceId || ''), path: String(node?.data.artifact?.path || '') });
      if (!extensionHostAcceptanceEnabled()) await vscode.window.showErrorMessage(`Tiinex Attach blocked: ${shortMessage(error)}`);
    }
  }
  ...
  private async packageOutgoing(): Promise<void> {
    ...
    if (!routes.length) {
      ...
      try {
        ...
        await vscode.window.withProgress(
          { location: vscode.ProgressLocation.Notification, title: 'Tiinex finishing Workspace carrier', cancellable: false },
          async (progress) => {
            progress.report({ message: 'Refreshing packaged carrier discovery…' });
            await this.refreshDiscoveryAfterPack(built.outputPath);
            progress.report({ message: 'Queueing finished carrier for Transport…' });
    ...
    // Multi-Handoff routes are peer routes. Core still needs one mechanical route
    // argument to enter the shared projection, but the host must not ask the user
    // to declare a semantic "primary" route. Preserve attachment order only as
    // the deterministic mechanical anchor; Core owns route allocation for all routes.
    ...
    try {
        ...
        async (progress) => {
          ...
          try {
            ...
            return await buildHandoffPackageFromForm(this.extensionPath, {
              ...
              routeInputs: routes.map((item) => ({ routeId: routeChoiceKeyForHandoff(item.draft.workspaceId, item.draft.path), participantRoles: item.participantSelections, endpointRoles: item.endpointRoles })),
              workspaceIds,
      ...
      let transportWarning = '';
      await vscode.window.withProgress(
        { location: vscode.ProgressLocation.Notification, title: 'Tiinex finishing Handoff carrier', cancellable: false },
        async (progress) => {
          progress.report({ message: 'Refreshing packaged carrier discovery…' });
          await this.refreshDiscoveryAfterPack(built.outputPath);
          progress.report({ message: 'Queueing finished carrier for Transport…' });
          transportWarning = await this.queueBuiltTransportPackage(built.outputPath, true);
        }
      );
      this.closeOutgoing();
  ...
  private async logicalWorkspaceRootChildren(section: OperatorSection, workspaceId: string, packagePath = ''): Promise<OperatorNode[]> {
    ...
    if (hasPayload) {
      const artifacts = await this.rawWorkspaceArtifacts(section, workspaceId, packagePath);
      if (artifacts.length) nodes.push(logicalWorkspaceGroupNode(section, workspaceId, packagePath, 'feed', 'Feed'));
      nodes.push(logicalWorkspaceGroupNode(section, workspaceId, packagePath, 'files', 'Files'));
      if (artifacts.length) nodes.push(logicalWorkspaceGroupNode(section, workspaceId, packagePath, 'lineage', 'Lineage'));
    }
  ...
  private async logicalWorkspaceHandoffTargets(section: OperatorSection, workspaceId: string, packagePath: string): Promise<OperatorNode[]> {
    if (section === 'outgoing') {
      ...
      return this.outgoing.drafts
        ...
        .map((item) => {
          const participantState = item.participantProjectionState === 'qualified'
            ? `${item.participants.length} Core-qualified participant Role${item.participants.length === 1 ? '' : 's'}`
            : item.participantProjectionState === 'blocked'
              ? 'participant projection blocked'
              : 'participant authority unresolved';
          const participantLines = item.participants.map((participant) => `Participant Role: ${participant.label} → ${participant.reference}`);
          return new OperatorNode({
            kind: 'draft', section: 'outgoing', id: `outgoing:draft:${item.id}`, label: item.draft.title,
            description: item.writtenPath
              ? `${item.routeIncluded ? 'handoff · route included' : 'handoff · written'} · ${participantState}`
...
type LogicalWorkspaceGroupKind = 'feed' | 'lineage' | 'files';
...
function parseLogicalWorkspaceGroup(value: string): { workspaceId: string; kind: LogicalWorkspaceGroupKind } | null {
  const match = String(value || '').match(/^(.+):logical:(feed|lineage|files)$/);
  ...
  if (fullLineage && draft.participantProjectionState === 'qualified') {
    for (const participant of draft.participants) {
      nodes.push(projectedPendingSemanticNode(
        ...
        'Core-qualified · final carrier path pending Pack',
        draft.draft.workspaceId,
        { contextValue: 'tiinex.outgoingParticipantRolePointerProjected', draftId: draft.id, pointerKind: 'participant' }
      ));
    }
  }
  if (fullLineage) {
...
function projectedPendingSemanticNode(
  label: string,
  description: string,
  workspaceId: string,
  options: { contextValue?: string; draftId?: string; pointerKind?: 'participant' | 'handoff' } = {}
): OperatorNode {
  return new OperatorNode({
    kind: 'projectedFile', section: 'outgoing', id: `outgoing:pending-carrier:${workspaceId}:${label}`,
    label, description,
    tooltip: `${label}\n${description}\nShared Core manufacture owns the final carrier bytes, cache decision and pointer filename.`,
    workspaceId,
