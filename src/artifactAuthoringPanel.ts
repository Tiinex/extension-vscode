...
<div class="grid"><label class="field"><span>Workspace</span><select id="workspace">${workspaceOptions}</select></label><label class="field"><span>Title <span class="required">required</span></span><input id="title" autocomplete="off" /></label>${templates.length ? `<label class="field"><span>Preset</span><select id="template">${templateOptions}</select><div class="template-help" id="templateHelp"></div></label>` : ''}<div class="field"><span>Host action</span>${attach}</div></div>${endpointControls}${parent}</div>
${assistLists}
...
function setEndpointRawVisible(field,visible){for(const label of endpointRawLabels(field))label.classList.toggle('endpoint-raw-hidden',!visible);for(const label of endpointReferenceLabels(field))label.classList.add('endpoint-raw-hidden')}
for(const select of qa('[data-endpoint-assist]')){const field=select.dataset.endpointAssist;const assist=fieldAssists.find(item=>item.field===field);if(!assist)continue;setEndpointRawVisible(field,false);select.addEventListener('change',()=>{if(select.value==='__manual__'){setEndpointRawVisible(field,true);exactSetField(field+' Reference','');return}setEndpointRawVisible(field,false);if(select.value===''){exactSetField(field+' Reference','');return}const suggestion=assist.suggestions[Number(select.value)];if(!suggestion)return;exactSetField(field,suggestion.value||suggestion.label||'');for(const [target,value] of Object.entries(suggestion.fills||{}))if(allField(target).length)exactSetField(target,value)})}
...
for(const select of qa('[data-endpoint-assist]')){const field=select.dataset.endpointAssist;const assist=fieldAssists.find(item=>item.field===field);if(!assist)continue;const reference=String(initialValues?.[field+' Reference']||'').trim();const index=reference?(assist.suggestions||[]).findIndex(item=>String(item.reference||'').trim()===reference):-1;if(index>=0){select.value=String(index);setEndpointRawVisible(field,false);continue}if(String(initialValues?.[field]||'').trim()){select.value='__manual__';setEndpointRawVisible(field,true)}}
