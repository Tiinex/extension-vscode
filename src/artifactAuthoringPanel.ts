...
function endpointRawLabels(field){const names=[field,field+' Kind',field+' Reference'];return names.flatMap(name=>allField(name).map(el=>el.closest('label.field')).filter(Boolean))}
function setEndpointRawVisible(field,visible){for(const label of endpointRawLabels(field))label.classList.toggle('endpoint-raw-hidden',!visible)}
for(const select of qa('[data-endpoint-assist]')){const field=select.dataset.endpointAssist;const assist=fieldAssists.find(item=>item.field===field);if(!assist)continue;setEndpointRawVisible(field,false);select.addEventListener('change',()=>{if(select.value==='__manual__'){setEndpointRawVisible(field,true);return}setEndpointRawVisible(field,false);if(select.value==='')return;const suggestion=assist.suggestions[Number(select.value)];if(!suggestion)return;exactSetField(field,suggestion.value||suggestion.label||'');for(const [target,value] of Object.entries(suggestion.fills||{}))if(allField(target).length)exactSetField(target,value)})}
