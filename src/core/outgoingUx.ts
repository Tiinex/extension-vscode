 ...
 */
export function inheritedOutgoingLabel(filename: string, carrierDimension: string): string {
  const dimension = String(carrierDimension || '').trim();
  const stem = String(filename || '').trim().replace(/\.handoff-package\.zip$/i, '');
  if (!dimension) return stem.toLocaleLowerCase();
  if (stem === dimension || stem.startsWith(`${dimension}-`)) return dimension.toLocaleLowerCase();
  const marker = `-${dimension}-`;
  const markerIndex = stem.indexOf(marker);
  const prefix = markerIndex >= 0 ? stem.slice(0, markerIndex) : '';
  return `${prefix ? `${prefix}-` : ''}${dimension}`.toLocaleLowerCase();
}
