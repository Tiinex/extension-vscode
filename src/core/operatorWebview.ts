...
q('#useReceivedControl')?.addEventListener('click',()=>vscode.postMessage({type:'useReceivedControl',controlId:q('#receivedControl')?.value||''}));
...
q('#copyRouting')?.addEventListener('click',()=>vscode.postMessage({type:'copyRouting'}));
...
else if(route&&model.preferredRouteId&&model.routes.some(r=>r.id===model.preferredRouteId))route.value=model.preferredRouteId;
...
else if(Array.isArray(model.defaultWorkspaceIds)){document.querySelectorAll('input[name=workspace]').forEach(x=>{x.checked=model.defaultWorkspaceIds.includes(x.value);});}
